...
// Define particle interface to replace any[] type
interface Particle {
  x: number;
  y: number;
  originX: number;
...
const ParticleSystem: React.FC<ParticleSystemProps> = ({
  ...
  useEffect(() => {
    ...
    const createParticles = (): Particle[] => {
      ...
      if (!sectionRef.current || !canvas) return particles;
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((particle) => {
        ...
        // Add wave motion
        const waveX =
          Math.sin(time * particle.wave.frequency + particle.wave.offset) *
          particle.wave.amplitude;
        const waveY =
          Math.cos(time * particle.wave.frequency + particle.wave.offset) *
          particle.wave.amplitude;

        // Add turbulence
        particle.turbulence.x +=
          (Math.random() * 2 - 1) * particle.turbulence.changeRate;
        particle.turbulence.y +=
          (Math.random() * 2 - 1) * particle.turbulence.changeRate;
