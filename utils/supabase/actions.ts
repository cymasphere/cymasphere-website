...
export async function fetchIsAdManager(
  id: string
): Promise<{ is_ad_manager: boolean; error: PostgrestError | null }> {
  const supabase = await createSupabaseServer();

  const { data, error } = await supabase
    .from("admins")
    .select("is_ad_manager")
    .eq("user", id)
    .single();

  if (error) {
    // If no admin record exists, user is not an ad manager
    if (error.code === 'PGRST116') {
      return { is_ad_manager: false, error: null };
    }
    return { is_ad_manager: false, error };
  }

  return { is_ad_manager: data?.is_ad_manager === true, error: null };
}
...
export async function fetchUserPermissions(
  ...
  // Check admin table for user permissions
  ...
  const isAdManagerOnly = adminData?.is_ad_manager === true;
  const isFullAdmin = hasAdminRecord && !isAdManagerOnly;
  
  return { 
    is_admin: isFullAdmin,
    is_ad_manager: isAdManagerOnly,
    can_access_admin: isFullAdmin, // Only full admins can access admin console
    can_access_ad_manager: hasAdminRecord, // Both full admins and ad managers can access ad manager
    error: null 
  };
}
