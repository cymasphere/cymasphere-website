...
export const initAudio = async () => {
  ...
  try {
    ...
    const impulseResponse = createImpulseResponse(3, 2.5); // 3 seconds decay, 2.5 pre-delay
    convolver.buffer = impulseResponse;
    
    // Connect convolver to output
    convolver.connect(audioContext.destination);
    
    console.log("Simple Web Audio API initialized with state:", audioContext.state);
    initialized = true;
...
// Create impulse response for reverb
function createImpulseResponse(duration, predelay) {
...
function playNote(frequency, duration, delay, velocity) {
  if (!audioContext) return;
  
  // Create oscillators for richer sound
  const oscillators = [
    { type: 'sine', gain: 0.7 },   // Main sine wave
    { type: 'triangle', gain: 0.2 },  // Add some triangle for harmonics
    { type: 'square', gain: 0.05 }   // Tiny amount of square for edge
  ];
...
const buildChordVoicing = (rootNote, chordType, extensions = []) => {
...
export const playChordPad = async (chordRoot) => {
  if (!initialized) {
    await initAudio();
  }
  
  // Specific chord type mapping for the chord buttons
  const specificChordMap = {
    // 7th chords
  ...
  const chordConfig = specificChordMap[chordRoot] || { root: 'C4', type: 'major' };
  ...
  const bassFifth = Tone.Frequency(rootNote).transpose(-12 + 7).toNote();
  ...
  sortedVoicing.forEach((note, index) => {
    const noteVelocity = index === sortedVoicing.length - 1 ? 0.36 : 0.6 * (1 - (index * 0.08));
...
export const playLydianMaj7Chord = async () => {
  ...
  const chordNotes = randomChordType.intervals.map(interval => randomRoot + interval);
  ...
  console.log(`Logo click: Playing random ${rootName} ${randomChordType.name} chord (active: ${activeLydianChords})`);
  ...
  synth.triggerAttack(Tone.Frequency(bassNote, "midi").toFrequency(), Tone.now(), 0.42);
  
  // Play the fifth after a short delay
  synth.triggerAttack(Tone.Frequency(fifthNote, "midi").toFrequency(), Tone.now() + 0.03, 0.28);
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      activeLydianChords--;
      console.log(`Chord cleanup complete. Active chords: ${activeLydianChords}`);
...
const playKick = (time = 0) => {
  ...
  osc.frequency.exponentialRampToValueAtTime(40, audioContext.currentTime + time + 0.3);
  
  // Set up volume envelope - INCREASED VOLUME
  oscGain.gain.value = 1.2; // Increased from 0.8 to 1.2
  oscGain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + time + 0.4);
...
const playSnare = (time = 0) => {
  ...
  noiseGain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + time + 0.2);
  ...
  oscGain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + time + 0.1);
...
const playHihat = (time = 0, isOpen = false) => {
  if (!initialized) return;
  ...
  hihatGain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + time + duration);
...
let drumPatternInterval = null;

// Play a basic drum pattern alongside the chord progression
export const playDrumPattern = (bpm = 120) => {
  if (!initialized) {
    initAudio();
  }
  ...
  console.log("Starting drum pattern at", bpm, "BPM, 16th note interval:", secondsPer16th * 1000, "ms");
  
  // Keep track of current 16th note position
  let step = 0;
  
  // Create interval to play the pattern
  drumPatternInterval = setInterval(() => {
    // Play kick drum
    ...
    if (HIHAT_PATTERN.includes(step % 16)) {
      ...
      console.log(`Playing ${isOpen ? 'open' : 'closed'} hihat at step`, step % 16);
      playHihat(0, isOpen);
    }
