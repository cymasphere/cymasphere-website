...
const PianoRoll = ({ onNotesChange, onNotePlay, initialNotes = [], externalPlaybackPosition = null }) => {
  ...
  const config = {
    ...
    totalBars: 4,
    isBlackKey: {
      'C': false, 'C#': true, 'Db': true,
      'D': false, 'D#': true, 'Eb': true,
      'E': false, 
      'F': false, 'F#': true, 'Gb': true,
      'G': false, 'G#': true, 'Ab': true,
      'A': false, 'A#': true, 'Bb': true,
      'B': false
    },
    noteColors: {
      'C': '#8A2BE2',
  ...
  const handleGridMouseDown = (e) => {
    ...
    if (!isDrawing) {
      ...
      const startBeat = Math.floor((x / rect.width) * (totalBeats * sixteenthsPerBeat));
      ...
      if (startBeat >= 0 && startBeat < config.cols && noteIndex >= 0 && noteIndex < config.noteNames.length) {
  ...
  const playSequence = () => {
    ...
    const maxPosition = Math.max(
      ...
      config.cols - 1 // Ensure we reach the end of the grid
    );
    
    // Play notes column by column with a more accurate interval for musically-timed playback
    // 16th notes at 120 BPM = 125ms between notes
    const interval = 125; 
    ...
    playbackRef.current = setInterval(() => {
      ...
      if (notesByPosition[currentPosition]) {
        notesByPosition[currentPosition].forEach(note => {
          if (onNotePlay) {
            onNotePlay(note.noteName);
          }
        });
      ...
      // Stop at the end of the sequence or when reaching the maximum position
  ...
  const stopSequence = () => {
    ...
    setIsPlaying(false);
    setPlaybackPosition(-1); // Hide the playhead
  };
  ...
  useEffect(() => {
    if (externalPlaybackPosition !== null) {
      ...
      setIsPlaying(externalPlaybackPosition > 0);
    }
  ...
  const playheadStyle = {
    position: 'absolute',
    left: `${((externalPlaybackPosition !== null ? externalPlaybackPosition : playbackPosition) / config.cols) * 100}%`,
    top: 0,
        ...
        {config.noteNames.map((note, index) => {
          if (!isBlackKey(note)) {
            ...
            return (
              ...
              >
                {noteName}<span style={{ fontSize: '0.5rem', marginLeft: '1px' }}>{octave}</span>
              </div>
            );
          }
          return null;
  ...
  // Render time divisions (minimized since we're removing the header)
  const renderTimeDivisions = () => {
    // Return nothing since we're removing the header completely
    return null;
  };
        ...
        {Array.from({ length: 16 }).map((_, index) => (
          <div 
            ...
            style={{
              ...
              width: '1px',
              height: '100%',
              backgroundColor: index % 4 === 0 ? 'transparent' : 'rgba(255, 255, 255, 0.15)',
              zIndex: 0,
              pointerEvents: 'none',
            }}
          />
  ...
  return (
    <div className="piano-roll" style={{ 
      ...
      {/* No time divisions since we're removing the header */}
        ...
        {notes.map(note => (
          <div 
            ...
            style={{
              ...
              top: `${config.noteNames.indexOf(note.noteName) * config.noteHeight}px`,
