...
const SynthSelector = ({ selectedSynth, onSynthChange, midiOutputs, selectedOutput, onOutputChange, selectedPreset, onPresetChange }) => {
  // Create a handler that directly passes the synth type string
  const handleSynthChange = (synthType) => {
  ...
  return (
    <div style={{
      ...
      {selectedSynth === 'internal' && midiOutputs && midiOutputs.length > 0 && (
        <select
      ...
      {selectedSynth === 'polysynth' && (
        ...
        >
          <option value="" disabled>Select Synth Preset</option>
