...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (search && search.length >= 2) {
      ...
      const profileSubscriberMatches = profileUserIds.length > 0 ? await supabase
        .from('subscribers')
        .select('id')
        .in('user_id', profileUserIds) : { data: [] };
    ...
    const { data: subscribers, error: subscribersError, count } = await subscribersQuery;
    ...
    const stats = {
      ...
      unsubscribed: statsData?.filter(s => s.status === 'unsubscribed').length || 0,
      bounced: statsData?.filter(s => s.status === 'bounced').length || 0,
      pending: statsData?.filter(s => s.status === 'pending').length || 0,
      highEngagement: Math.floor((statsData?.filter(s => s.status === 'active').length || 0) * 0.3), // Mock high engagement
      growthRate: "12%" // Mock growth rate
    };

    // Get profile data for subscribers to include names
    const userIds = subscribers?.filter(s => s.user_id).map(s => s.user_id!).filter(Boolean) || [];
    ...
    const transformedSubscribers = (subscribers || []).map(subscriber => {
      ...
      const fullName = [firstName, lastName].filter(Boolean).join(' ') || subscriber.email.split('@')[0];
      ...
      return {
        ...
        engagement: Math.random() > 0.5 ? 'High' : Math.random() > 0.5 ? 'Medium' : 'Low',
