...
export async function PUT(
  ...
  try {
    ...
    if (audienceInserts.length > 0) {
      ...
      if (audienceError) {
        console.error(
          "🚨 Database error updating campaign audiences:",
          audienceError
        );
        // Don't fail the entire request, just log the error
        console.warn(
...
export async function DELETE(
  ...
  try {
    ...
    if (campaign.status === "sent") {
      console.log(
        `⚠️ IMPORTANT: Deleting SENT campaign - this will remove all data and analytics for "${campaign.name}"`
      );
    }
    ...
    if (campaign.status === "sent") {
      ...
      if (sendsDeleteError) {
        console.error(
          "Error deleting campaign send records:",
          sendsDeleteError
        );
    ...
    if (audienceDeleteError) {
      console.error(
        "Error deleting campaign audience relationships:",
        audienceDeleteError
      );
      // Continue with deletion anyway - this is not critical
    }
