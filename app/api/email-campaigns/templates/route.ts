...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (search) {
      query = query.or(
        `name.ilike.%${search}%,description.ilike.%${search}%,subject.ilike.%${search}%`
      );
    }
    ...
    if (templateIds.length > 0) {
      ...
      campaigns?.forEach((campaign: any) => {
        if (campaign.template_id) {
          usageStats[campaign.template_id] =
            (usageStats[campaign.template_id] || 0) + 1;
        }
      });
    }
      ...
      if (audienceError) {
        ...
        audienceRelations?.forEach((relation: any) => {
          ...
          if (relation.is_excluded) {
            templateAudiences[relation.template_id].excluded.push(
              relation.audience_id
            );
          } else {
            templateAudiences[relation.template_id].included.push(
              relation.audience_id
            );
          }
        });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (audienceInserts.length > 0) {
      ...
      if (audienceError) {
        console.error(
          "🚨 Database error creating template audiences:",
          audienceError
        );
        console.error(
          "🚨 Audience error details:",
          JSON.stringify(audienceError, null, 2)
        );
        // Don't fail the entire request, just log the error
        console.warn("⚠️ Template created but audience relationships failed");
      } else {
        console.log(
          `✅ Created ${audienceInserts.length} template audience relationships`
        );
      }
    } else {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (deleteError) {
      console.error(
        "🚨 Database error deleting existing template audiences:",
        deleteError
      );
      // Don't fail the entire request, just log the error
      console.warn(
        "⚠️ Template updated but failed to delete existing audience relationships"
      );
    }
    ...
    if (audienceInserts.length > 0) {
      ...
      if (audienceError) {
        console.error(
          "🚨 Database error updating template audiences:",
          audienceError
        );
        // Don't fail the entire request, just log the error
        console.warn(
          "⚠️ Template updated but audience relationships may have failed"
        );
      } else {
        console.log(
          `✅ Updated ${audienceInserts.length} template audience relationships`
        );
      }
    }
