...
export async function GET() {
  try {
    ...
    const reputation =
      totalSent > 0
        ? Math.max(0, Math.min(100, 100 - bounceRate - spamRate))
        : 0;
    ...
    const topDomains = Array.from(domainStats.entries())
      ...
      .map(([domain, stats], index) => {
        ...
        const reputation =
          stats.total > 0 ? Math.max(0, Math.min(100, deliveryRate)) : 0;
        ...
        return {
          ...
          dkimStatus:
            stats.total > 0 && deliveryRate > 90 ? "valid" : "unknown",
          spfStatus: stats.total > 0 && deliveryRate > 85 ? "pass" : "unknown",
          dmarcStatus:
            stats.total > 0 && deliveryRate > 80 ? "pass" : "unknown",
    ...
    const recentBounces =
      emailSends
        ...
        .map((send, index) => {
          ...
          const campaignName =
            campaigns?.find((c) => c.id === send.campaign_id)?.name ||
            "Unknown Campaign";
          ...
          return {
            ...
            type:
              send.bounce_reason?.includes("permanent") ||
              send.bounce_reason?.includes("does not exist")
                ? "hard"
                : "soft",
            reason: send.bounce_reason || "Unknown bounce reason",
            campaign: campaignName,
            timestamp:
              send.bounced_at || send.sent_at || new Date().toISOString(),
          };
        }) || [];
