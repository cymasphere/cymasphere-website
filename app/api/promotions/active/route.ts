...
export async function GET(request: NextRequest) {
  try {
    ...
    let suggested_promo_code: string | null = null;
    if (activePromotion?.stripe_coupon_code && stripe) {
      try {
        ...
        if (list.data[0]?.code) {
          suggested_promo_code = list.data[0].code;
        }
      } catch {
        // Stripe lookup failed; omit suggested code
      }
    }
    ...
    return NextResponse.json({
      ...
      suggested_promo_code: suggested_promo_code ?? undefined,
