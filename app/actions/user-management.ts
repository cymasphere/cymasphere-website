...
/**
 * @brief Download a support attachment and extract readable text (admin only).
 * @param attachmentId support_attachments row id.
 * @returns Extracted text files from .txt/.log or zip entries (e.g. crash stacks).
 * @note Used so admins can copy stack traces without downloading and unzipping.
 */
export async function getSupportAttachmentTextPreview(attachmentId: string): Promise<{
  success: boolean;
  fileName?: string;
  ...
  try {
    ...
    const serviceSupabase = await createSupabaseServiceRole();
    const { data: attachment, error } = await serviceSupabase
      .from("support_attachments")
      .select("id, file_name, file_type, storage_path, attachment_type")
      .eq("id", attachmentId)
      .single();
    ...
    } = await import("@/lib/support/extract-attachment-text");
    ...
    if (
      ...
      return {
        success: false,
        error: "This attachment type cannot be previewed as text",
      };
    }

    let downloadPath = attachment.storage_path;
    if (downloadPath.startsWith("support-attachments/")) {
      downloadPath = downloadPath.replace("support-attachments/", "");
    }
    ...
    if (downloadError || !blob) {
      console.error(
        "[getSupportAttachmentTextPreview] download failed:",
        downloadError,
        { downloadPath, storage_path: attachment.storage_path }
      );
      // Retry with raw storage_path in case it was stored without the prefix convention
      const retry = await serviceSupabase.storage
        .from("support-attachments")
        .download(attachment.storage_path);
      if (retry.error || !retry.data) {
        return { success: false, error: "Failed to download attachment" };
      }
      const retryBytes = new Uint8Array(await retry.data.arrayBuffer());
      const retryExtracted = extractAttachmentText(
        attachment.file_name || "attachment",
        attachment.file_type || "",
        retryBytes
      );
      if (retryExtracted.files.length === 0) {
        return {
          success: false,
          error: "No readable text found in this attachment",
        };
      }
      return {
        success: true,
        fileName: attachment.file_name || "attachment",
        files: retryExtracted.files,
        combined: retryExtracted.combined,
      };
    }
    ...
    console.error("Error in getSupportAttachmentTextPreview:", error);
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to preview attachment",
    };
  }
}
...
export async function createSupportTicket(data: {
  ...
  try {
    ...
    /**
     * @note Admin inbox email is sent inside createTypedSupportTicket for every
     * ticket type (support, bug, feature, crash) across web and API entry points.
     */
    return {
      success: true,
