...
export const applyTonePatches = () => {
  console.log("Applying radical Tone.js patches to prevent undefined parameter issues");
  ...
  try {
    ...
    Tone.FMSynth.prototype.triggerAttack = function(note, time, velocity = 1) {
      ...
      console.log(`FMSynth.triggerAttack override called: note=${note}, time=${time}, velocity=${velocity}`);
      
      try {
        // Get absolute time
        const now = this.context.currentTime;
        const computedTime = time !== undefined && time !== null ? this.toSeconds(time) : now;
        const computedVelocity = velocity !== undefined && velocity !== null ? velocity : 1;
        ...
        try {
          ...
          if (this.modulationIndex && typeof this.modulationIndex.value === 'number') {
          ...
          if (this.modulationEnvelope) {
            this.modulationEnvelope.triggerAttack(computedTime, computedVelocity);
          }
          
          return this;
        } catch (e) {
          console.error("Error in FMSynth.triggerAttack direct parameter setting:", e);
          return this;
        }
      } catch (e) {
    ...
    const originalPolySynthTriggerAttack = Tone.PolySynth.prototype.triggerAttack;
    Tone.PolySynth.prototype.triggerAttack = function(notes, time, velocity) {
      try {
        // Simple null check for parameters
        const safeTime = time !== undefined && time !== null ? time : Tone.now();
        const safeVelocity = velocity !== undefined && velocity !== null ? velocity : 1;
        ...
        return originalPolySynthTriggerAttack.call(this, safeNotes, safeTime, safeVelocity);
      } catch (e) {
